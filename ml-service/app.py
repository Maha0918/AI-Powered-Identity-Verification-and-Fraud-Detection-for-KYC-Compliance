...
DATA_DIR = os.path.join(PROJECT_ROOT, "Extracted data Records")
...
def classify():
    ...
    try:
        ...
        # ── Step 2: OCR + GNN ─────────────────────────────────────────────
        try:
            # Extract text using EasyOCR and Groq
            raw_text = extract_text_easyocr(tmp.name)
            ocr = extract_with_groq(raw_text, document_type)
            ...
            result["ocr_data"]       = {"error": str(ocr_err)}
